...
cp /app/stack/php/php.ini /etc/php/7.0/cli/php.ini
for configfile in /app/stack/php/conf.d/*; do
    cp $configfile /etc/php/7.0/cli/conf.d/
done
...
if [ ! "$LOAD_XDEBUG_EXTENSION" == "1" ]; then
  rm /etc/php/7.0/cli/conf.d/*xdebug*
fi
sed -i "s|\${DISPLAY_PHP_ERRORS}|${DISPLAY_PHP_ERRORS}|" /etc/php/7.0/cli/conf.d/app.ini
sed -i "s|\${XDEBUG_DEFAULT_ENABLE}|${XDEBUG_DEFAULT_ENABLE}|" /etc/php/7.0/cli/conf.d/app.ini
